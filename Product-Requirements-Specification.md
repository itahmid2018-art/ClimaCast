...
### 7.2 Native Android / iOS via Capacitor & GitHub Actions CI/CD
- `capacitor.config.json` configured with application ID `com.climacast.weather` and app name `ClimaCast`.
...
- **Automated CI/CD**:
  - Android APK (`climacast-android-*.apk`) automatically compiled via Gradle in GitHub Actions (`.github/workflows/release.yml`) on Ubuntu with Java 21 JDK and Android SDK.
  - iOS Simulator bundle (`climacast-ios-simulator-*.zip`) and native Xcode workspace project (`climacast-ios-xcode-project-*.zip`) automatically built on macOS 14 runners with Xcode.
  - Public release assets published automatically to GitHub Releases with SHA-256 integrity checksums.
