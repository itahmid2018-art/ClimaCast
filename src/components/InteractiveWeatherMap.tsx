...
export type WindyOverlay = 'radar' | 'satellite' | 'rain' | 'wind' | 'temp';
...
export const InteractiveWeatherMap: React.FC<InteractiveWeatherMapProps> = ({
  ...
  // Engine: 'windy' (Non-API, 100% free global live radar/satellite) or 'leaflet' (classic Leaflet tiles)
  const [engine, setEngine] = useState<'windy' | 'leaflet'>(() => {
    return (localStorage.getItem('gw_radar_engine') as 'windy' | 'leaflet') || 'windy';
  });

  // Windy Overlay state
  const [windyOverlay, setWindyOverlay] = useState<WindyOverlay>('radar');
  ...
  const [provider, setProvider] = useState<'rainviewer' | 'openweathermap'>(() => {
    return (localStorage.getItem('gw_radar_provider') as 'rainviewer' | 'openweathermap') || 'rainviewer';
  });
  const [hasOpenWeatherKey, setHasOpenWeatherKey] = useState<boolean>(false);
  ...
  const metricTemp = unit === 'fahrenheit' ? '°F' : '°C';
  const metricWind = unit === 'fahrenheit' ? 'mph' : 'km/h';
  const metricRain = unit === 'fahrenheit' ? 'in' : 'mm';

  const product =
    windyOverlay === 'radar'
      ? 'radar'
      : windyOverlay === 'satellite'
      ? 'satellite'
      : 'ecmwf';

  const windyUrl = `https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=${metricRain}&metricTemp=${metricTemp}&metricWind=${metricWind}&zoom=7&overlay=${windyOverlay}&product=${product}&level=surface&lat=${location.latitude}&lon=${location.longitude}&detailLat=${location.latitude}&detailLon=${location.longitude}&marker=true&message=true`;

  // Check OpenWeatherMap key availability from server
  useEffect(() => {
    fetch('/api/settings/keys')
      ...
      .then((data) => {
        ...
        const key =
          ...
          keys.RADAR_API_KEY ||
          localStorage.getItem('gw_radar_api_key');
        setHasOpenWeatherKey(!!key);
      })
      .catch(() => {
        setHasOpenWeatherKey(!!localStorage.getItem('gw_radar_api_key'));
      });
  }, []);

  // Fetch RainViewer public API frames for fallback Leaflet engine
  useEffect(() => {
    ...
    const fetchRadarMetadata = async () => {
      try {
        ...
        console.warn('RainViewer API error, using fallback:', err);
        const fallbackTime = Math.floor(Date.now() / 600000) * 600;
    ...
    fetchRadarMetadata();
    const interval = setInterval(fetchRadarMetadata, 10 * 60 * 1000);
    return () => {
      isMounted = false;
  ...
  // Initialize Leaflet Map (only when engine === 'leaflet')
  useEffect(() => {
    ...
    if (!mapInstanceRef.current) {
      ...
      L.control
        ...
        .addAttribution(
          '&copy; <a href="https://openweathermap.org/" target="_blank">OpenWeatherMap</a>, &copy; <a href="https://www.openstreetmap.org/copyright">OSM</a>, &copy; <a href="https://www.rainviewer.com/">RainViewer</a>'
        )
        .addTo(map);
  ...
  // Update Base Map Tiles for Leaflet
  useEffect(() => {
    if (engine !== 'leaflet') return;
  ...
  }, [engine, location.latitude, location.longitude, location.name, weather.current.temperature, unit]);

  // Update Weather Overlay Tile Layers (Leaflet)
  useEffect(() => {
    if (engine !== 'leaflet') return;
    const map = mapInstanceRef.current;
    if (!map) return;
    ...
    if (provider === 'openweathermap') {
      if (activeLayer === 'clouds' || activeLayer === 'both') {
        const satUrl = `/api/weather/tiles/clouds_new/{z}/{x}/{y}.png`;
      ...
      if (activeLayer === 'precipitation' || activeLayer === 'both') {
        const radarUrl = `/api/weather/tiles/precipitation_new/{z}/{x}/{y}.png`;
  ...
  }, [engine, provider, activeLayer, opacity, currentFrameIndex, radarFrames, satelliteFrames, apiHost]);

  // Leaflet Animation Loop
  useEffect(() => {
    if (engine !== 'leaflet' || !isPlaying || radarFrames.length <= 1) return;
  ...
  }, [engine, isPlaying, radarFrames.length]);

  // Fullscreen toggle
  const toggleFullscreen = () => {
    setIsFullscreen((prev) => !prev);
    setTimeout(() => {
      if (engine === 'leaflet' && mapInstanceRef.current) {
  ...
  // Current frame time label (Leaflet)
  ...
  return (
    <div
      ...
      className={`relative overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-xs transition-all dark:border-slate-800 dark:bg-slate-900 ${
        isFullscreen ? 'fixed inset-3 z-50 rounded-2xl shadow-2xl md:inset-6' : 'w-full'
      }`}
    >
      {/* Map Header Controls */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200/80 p-4 dark:border-slate-800">
        <div className="flex items-center gap-2.5">
          <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-blue-50 text-blue-600 dark:bg-blue-950/60 dark:text-blue-400">
            {engine === 'windy' ? <Globe className="h-4 w-4" /> : <Layers className="h-4 w-4" />}
          </div>
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-800 dark:bg-emerald-950/70 dark:text-emerald-300">
                ...
                {engine === 'windy' ? 'Windy Global (Keyless)' : 'Leaflet'}
              </span>
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Live Doppler radar, satellite cloud feeds, and winds for {location.name}
            </p>
          </div>
        </div>

        {/* Engine Switcher and Layer Selectors */}
        <div className="flex flex-wrap items-center gap-2">
          {/* Primary Engine Toggle: Windy vs Leaflet */}
          <div className="flex items-center rounded-2xl bg-slate-100 p-1 dark:bg-slate-800 text-xs">
            <button
              type="button"
              onClick={() => {
                setEngine('windy');
                localStorage.setItem('gw_radar_engine', 'windy');
              }}
              className={`flex items-center gap-1.5 rounded-xl px-2.5 py-1.5 font-semibold transition ${
                engine === 'windy'
                  ? 'bg-white text-blue-600 shadow-xs dark:bg-slate-700 dark:text-blue-300'
                  : 'text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white'
              }`}
              title="Windy provides global coverage, live Doppler radar, satellites, and wind with zero API keys."
            >
              <Globe className="h-3.5 w-3.5 text-blue-500" />
              <span>Windy (Global)</span>
            </button>
            ...
            <button
              ...
              onClick={() => {
                setEngine('leaflet');
                localStorage.setItem('gw_radar_engine', 'leaflet');
              }}
              className={`flex items-center gap-1.5 rounded-xl px-2.5 py-1.5 font-semibold transition ${
                engine === 'leaflet'
                  ? 'bg-white text-blue-600 shadow-xs dark:bg-slate-700 dark:text-blue-300'
              ...
              title="Classic Leaflet tile map"
            >
              <Layers className="h-3.5 w-3.5" />
              <span>Classic Leaflet</span>
            </button>
          </div>
              ...
              <div className="flex items-center rounded-2xl bg-slate-100 p-1 dark:bg-slate-800 text-xs">
                <button
                  ...
                  onClick={() => {
                    ...
                    localStorage.setItem('gw_radar_provider', 'rainviewer');
                  }}
                  className={`flex items-center gap-1.5 rounded-xl px-2.5 py-1.5 font-semibold transition ${
                    provider === 'rainviewer'
                      ? 'bg-white text-blue-600 shadow-xs dark:bg-slate-700 dark:text-blue-300'
                ...
                {hasOpenWeatherKey && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      localStorage.setItem('gw_radar_provider', 'openweathermap');
                    }}
                    className={`flex items-center gap-1.5 rounded-xl px-2.5 py-1.5 font-semibold transition ${
      ...
      <div className="relative">
        {engine === 'windy' ? (
          /* Windy Embed Map */
          <div
            className={`relative w-full bg-slate-900 transition-all ${
              isFullscreen ? 'h-[calc(100vh-160px)]' : 'h-[460px] sm:h-[500px]'
            }`}
          >
            {isIframeLoading && (
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-2 bg-slate-900/80 text-white backdrop-blur-xs">
                <Loader2 className="h-8 w-8 animate-spin text-blue-400" />
                <span className="text-xs font-medium text-slate-300">
                  Loading Live Global Weather Radar...
                </span>
              </div>
            )}
            <iframe
              key={`${iframeKey}-${windyOverlay}-${location.latitude}-${location.longitude}`}
              title="Windy Weather Radar and Satellite"
              src={windyUrl}
          ...
          <div
            ...
            className={`w-full bg-slate-100 dark:bg-slate-950 transition-all ${
              isFullscreen ? 'h-[calc(100vh-180px)]' : 'h-80 sm:h-96 md:h-[440px]'
            }`}
          />
        )}

        {/* Floating Quick Action Tools (Recenter & Fullscreen) */}
        ...
        {/* Bottom Floating Bar (Leaflet Controls only) */}
        {engine === 'leaflet' && (
                ...
                <div className="flex items-center gap-2.5 py-1">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-slate-500 dark:text-slate-400">
                      High-definition radar precipitation & cloud coverage
                    </span>
                  </div>
                </div>
              ) : (
